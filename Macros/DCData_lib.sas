...
%macro DCData_lib( library, mprint=n, env=, macdef=, conf_drive=, local=y, remote=y );
  ...
  %if not &_remote_session %then %do;
    ...
    ** Create separate and concatenated libraries for data sets and format catalogs **;
    ...
    %if &c_exist %then %do;
      libname &prename._c "&conf_path\&library\Data";
      %let c_ref = 1;
    ...
    %if not( &local = N or &local = NO ) %then %do; 
      %if &l_exist %then %do;
        libname &prename._l "&_dcdata_l_path\&library\Data";
        %let l_ref = 1;
...
/* options nomprint nosymbolgen nomlogic; */
  ...
  %else %do;
    ...
    %**** Set up libraries for REMOTE session ****;
