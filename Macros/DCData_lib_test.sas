/**************************************************************************
 ...
 Modifications:
  ...
  06/16/26 PAT Updated code for new SAS1 setup.
**************************************************************************/

%macro DCData_lib_test( library, mprint=n, env=, macdef=, conf_drive=, local=y, remote=y, rreadonly= );
  ...
  %if not &_final_batch_submit %then %do;
    ...
    %**** Set up libraries for local or interactive session ****;
  ...
  %else %do;
    ...
    %**** Set up libraries for FINAL session ****;
